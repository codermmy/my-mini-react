 ...
 * }
 */
export function traceComponentRender(componentName: string, props: any): () => void {
  if (!DEBUG) return () => {};
 ...
 * }
 */
export function traceEvent(eventName: string, eventData: any): void {
  if (!DEBUG) return;
 ...
 */
export function printElementTree(element: any, depth = 0): void {
  if (!DEBUG) return;
