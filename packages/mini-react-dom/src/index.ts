 ...
 * @returns 创建的DOM节点
 */
function createDom(element: Element): Node {
  ...
  try {
    ...
    Object.entries(props).forEach(([name, value]) => {
      ...
      // 处理className
        ...
        domElement.setAttribute('class', value as string);
      }
      // 处理其他属性
      else {
    ...
    return document.createTextNode(`错误: ${(error as Error).message}`);
  }
}
 ...
 */
function unmountComponentAtNode(container: HTMLElement | null): boolean {
  if (!container) {
    return false;
  }
